...
var Server = function(bind_address,config,state) {
    ...
    this.removeClient = function(client) {
        ...
        log("Removing client "+client.fullAddress);
    };
    ...
    // optional 3rd argument is the password.
    this.setName = function(client, newname) {
        ...
        if(nametoroom[oldname] !== undefined) {
            nametoroom[newname] = nametoroom[oldname];
    ...
    this.sendServerUserCount = function(name) {
        var client = clientsbyname[name];
        client.receiveServerUserCount(clients.length);
    };

    this.sendRoomCount = function(name) {
        var client = clientsbyname[name];
        client.receiveRoomCount(rooms.length);
    };

    this.sendRoomUserCount = function(name) {
        var client = clientsbyname[name];
        client.receiveRoomUserCount();
    };

   
    this.sendServerUserList = function(name) {
        var client = clientsbyname[name];
        var names = [];
        for(var name in clientsbyname) {
            names.push(name);
    ...
    this.sendRoomList = function(name) {
        var client = clientsbyname[name];
        var roomlist = Object.keys(rooms);
        client.receiveRoomList(roomlist);
    };

    this.sendRoomUserList = function(name,room) {
        var client = clientsbyname[name];
    ...
    this.sendInboxCount = function(name) {
        var client = clientsbyname[name];
        client.receiveInboxCount(0);
    };
    ...
    // other
    this.joinRoom = function(name,room) {
